...
def collect_graph(mypath,output_path='files\graph.json',extension='.md',out_extension='',subdirs=True,ignore_in=['dirs','_site','_includes'],ignore_eq=['bubbles','README','.'],stub_path=None,additional_keys={},excluded_fields=None):
    ...
    for (this_file,this_fullpath,this_subdir) in zip(filenames,onlyfiles,onlysubdirs):
        ...
        if excluded_fields:
            fm_exists, fm_lines = has_front_matter(data)
            if fm_exists:
                fm = next(yaml.load_all(''.join(fm_lines), Loader=yaml.FullLoader))
                for field in excluded_fields:
                    if field not in fm:
                        continue
                    vals = fm[field] if isinstance(fm[field], list) else [fm[field]]
                    for v in vals:
                        for link in re.findall(r'\[\[(.*?)\]\]', str(v)):
                            while link in go_to:
                                go_to.remove(link)
        if subdirs:
...
def clean_published(root='./', extension='.md', ignore_in=None):
    """Remove empty 'published:' front matter lines so Jekyll doesn't treat them as published: false."""
    if ignore_in is None:
        ignore_in = []
    skip = ignore_in + ['.git']
    for dirpath, _, filenames in os.walk(root):
        ...
        for filename in filenames:
            ...
            with open(filepath, 'r', encoding='utf-8') as f:
                content = f.read()
            cleaned = re.sub(r'^published:\s*$', '', content, flags=re.MULTILINE)
            if cleaned != content:
                with open(filepath, 'w', encoding='utf-8') as f:
                    f.write(cleaned)
                print(f'Removed empty published: from {filepath}')

def normalize_paths(root='./', ignore_in=None):
    """Rename files and directories to lowercase with spaces replaced by hyphens."""
    if ignore_in is None:
        ignore_in = []
    skip = ignore_in + ['.git']
    for dirpath, dirnames, filenames in os.walk(root, topdown=False):
        ...
        for filename in filenames:
            ...
            if normalized != filename:
                os.rename(os.path.join(dirpath, filename), os.path.join(dirpath, normalized))
                print(f'Normalized: {os.path.join(dirpath, filename)} -> {normalized}')
        dirname = os.path.basename(dirpath)
        if not dirname or dirname in ('.', '..') or dirname.startswith('.') or dirname.startswith('_'):
            continue
        normalized_dir = dirname.lower().replace(' ', '-')
        if normalized_dir != dirname:
            parent = os.path.dirname(dirpath)
            os.rename(dirpath, os.path.join(parent, normalized_dir))
            print(f'Normalized dir: {dirpath} -> {normalized_dir}')

#%% Collector
out_extension=CFG['out_extension']
ignore_in = CFG['ignore_in']
ignore_eq = CFG['ignore_eq']
graph_excluded_fields = CFG.get('graph_excluded_fields', [])
normalize_paths('./', ignore_in=ignore_in)
clean_published('./', ignore_in=ignore_in)
graph_subs=collect_graph('./',out_extension=out_extension,output_path='graphs/graph-subdirs.json',ignore_in=ignore_in,ignore_eq=ignore_eq,subdirs=True,excluded_fields=graph_excluded_fields)
...
graph_subs=collect_graph('./',out_extension=out_extension,output_path='graphs/graph-subdirs.json',ignore_in=ignore_in,ignore_eq=ignore_eq,subdirs=True,additional_keys=additional_keys,excluded_fields=graph_excluded_fields)
graph_nosubs=collect_graph('./',out_extension=out_extension,output_path='graphs/graph.json',ignore_in=ignore_in,ignore_eq=ignore_eq,subdirs=False,additional_keys=additional_keys,excluded_fields=graph_excluded_fields)
